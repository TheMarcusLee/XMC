...
if(isset($_GET['sub_more_page']) || isset($_GET['action']) ){
    ...
    if($_GET['action'] == 'pause_session'){
        ...
        echo '<script>window.location.href="'.home_url().'/dashboard/?page=ringless_voice_mail&sub-page=campaigns"</script>';
    }
    ...
    if($_GET['action'] == 'resume_session'){
		...
		if($_GET['action'] == 'cancel_campaign'){
			...
			echo '<script>window.location.href="'.home_url().'/dashboard/?page=ringless_voice_mail&sub-page=campaigns"</script>';
				...
				echo '<script>window.location.href="'.home_url().'/dashboard/?page=ringless_voice_mail&sub-page=campaigns&sess=0"</script>';
			}
						...
						<div class="table-div">
							...
							<div class="table-box">
								<div class="table-heading">
									<h5>
										<div class="camp_headpart_left">
											Campaign List
										</div>
										<div class="camp_headpart_right">
										<small class="heading-earnings add-earning">
                                        <font color="#007803"><a href="<?php echo $_SERVER['REQUEST_URI'];?>&action=add_campaign" id="create_list" class="btn btn-primary pull-right" > <i class="fa fa-plus"></i> Add Campaign</a></font></small>
                                        <small class="heading-earnings add-earning">
											<font color="#007803"><a href="<?php echo $_SERVER['REQUEST_URI'];?>&action=campaign_status" id="create_list" class="btn btn-warning pull-right" >Running Campaign Status</a></font></small>
										</div>
									</h5>
									<hr />
								</div>
...
<!--                                         <td><?php echo $serial;?></td> -->
                                        ...
                                        <td> <?php if($camp['subscribers'] == '') { echo '0';}else{ echo $camp['subscribers']; } ?></td>
											<!-- <td><?php echo subscriberCount($camp['id'])->total;?></td> -->
											<td><?php echo $camp['schd_date'];?></td>
											<td><?php echo $camp['schd_time'];?></td>
											<?php if($camp['schd_date'] == NULL) { ?>
											<td>
                                            <?php 
                                            $ses =  $camp['session_id'];
                                            ...
                                            if($camp['check_status'] != 1){
												...
												if(strpos($camp['session_status'],'finished')  != false){ 
													?>
                                                ...
                                                ?>                                                                                                            
												<a href="javascript:void(0);" onclick="clone_campaign('<?php echo $camp['title']; ?>','<?php echo $camp['caller_id']; ?>',<?php echo $camp['phone_list']; ?>,'<?php echo $camp['audio_list'];?>','<?php echo $camp['camp_list'];?>');" data-toggle="tooltip" title="Copy Campaign"><i class="fa fa-clone" style="color:green; font-size:20px;"></i></a>
                                                    
											</td>
										<?php }else{ ?> <td><a href="<?php echo $_SERVER['REQUEST_URI'];?>&action=cancel_campaign&session_val=<?php echo $ar;?>&id=<?php echo $camp['id']; ?>"><i class="fa fa-trash-o fa-1x camp-action"></i></a></td>
										<?php }?>
                                            </tr>
                                        <?php 
                                        $serial++;
	...
	<script>
	function clone_campaign(title,caller_id,phone_list,audio_list,camp_list){
